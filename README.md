...
- `original_main_run/`: multilingual local-audit tables, prompt inventory, manual observation summaries, visual-feature tables, visual-drift tables, token-suffix sensitivity tables, and figure source files.
...
- `qwen_image_api_probe/`: 60-image probe through a tested OpenAI-compatible endpoint using model name `Qwen-Image`, including redacted metadata summaries, coding tables, reports, contact sheet, and metadata sidecars with private endpoint details removed.
- `conceptual_figures/`: GPT-image-2-generated conceptual figure output and prompt file used for the manuscript's prompt-mediation control-chain figure.
...
- The schema note in `docs/qwen_redacted_schema_note.md` documents the returned field hierarchy for the tested OpenAI-compatible endpoint using model name `Qwen-Image` without publishing credentials or private endpoint details.
- Visual feature and drift tables provide the quantitative summaries used for cross-language comparison.
- Token-suffix sensitivity tables document a tokenizer-only diagnostic that recounts submitted prompts before appending the shared English quality suffix. These tables quantify token-budget pressure and do not represent a without-suffix image-generation experiment.
...
These materials support a mechanism-oriented sociotechnical audit. They are not a model leaderboard, a comprehensive benchmark of image quality, or a claim that any platform lacks internal prompt transformation. In particular, the tested endpoint probe documents what one OpenAI-compatible endpoint, using model name `Qwen-Image`, exposed in returned metadata during this controlled run.

## Qwen-Image Scope Note

The tested-endpoint materials document exposed metadata from one OpenAI-compatible endpoint, one model name (`Qwen-Image`), one image size, and a 60-request controlled prompt set. Empty `revised_prompt` fields are evidence about metadata observability in this endpoint, not proof that no internal prompt transformation occurred in Qwen-Image or in other compatible endpoint implementations.

## Conceptual Figure Note

The conceptual control-chain figure in `conceptual_figures/gpt_image2/` was generated with GPT-image-2 for manuscript explanation. It is not reused from earlier local mechanism figures and is not itself an empirical result. Its purpose is to summarize the bounded mechanism logic: submitted prompt, representation pathway, possible system-side rewriting, metadata regime, and practical creative-control conditions.
