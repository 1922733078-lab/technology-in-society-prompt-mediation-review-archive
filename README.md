...
These materials support a mechanism-oriented sociotechnical audit. They are not a model leaderboard, a comprehensive benchmark of image quality, or a claim that any platform lacks internal prompt transformation. In particular, the Qwen-Image probe documents what one OpenAI-compatible endpoint exposed in returned metadata during this controlled run.

## Qwen-Image Scope Note

The Qwen-Image materials document exposed metadata from one OpenAI-compatible endpoint, one model name, one image size, and a 60-request controlled prompt set. Empty `revised_prompt` fields are evidence about metadata observability in this endpoint, not proof that no internal prompt transformation occurred.
